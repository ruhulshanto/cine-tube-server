...
async function main() {
  console.log("Starting forced PRISMA image update...");

  // Update EVERY movie that exists, overriding empty/black/broken URLs with robust Unsplash movie-vibe photos
  for (const movie of movies) {
      try {
        ...
        console.log(`✅ Success fixing: ${result.title}`);
      } catch (err) {
        console.log(`❌ Skipped (Not Found or Error): ${movie.slug}`);
      }
  }
  
  // As a fallback, update ANY remaining movies that somehow didn't fit slugs
  ...
  for (const m of allMovies) {
      if (!m.posterUrl || m.posterUrl.includes("cloudinary.com") || m.posterUrl.includes("tmdb.org")) {
           await prisma.movie.update({
             where: { id: m.id },
             data: { posterUrl: defaultPoster, backdropUrl: defaultBackdrop }
           });
           console.log(`✅ Default applied to: ${m.title}`);
      }
  }
